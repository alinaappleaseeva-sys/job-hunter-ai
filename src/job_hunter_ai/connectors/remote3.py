"""remote3.co connector.

Targets remote web3 jobs, with focus on operations/dao/governance roles.
Has both HTML pages and an RSS feed (/api/rss).
"""
...
_BASE = "https://remote3.co"
_SOURCE_TYPE = "job_board"
_PROVIDER = "remote3"


class Remote3Connector(Connector):
    """Scraper for remote3.co web3 job listings."""

    def __init__(
        self,
        source_name: str = "remote3",
        paths: List[str] | None = None,
        client: Any | None = None,
    ) -> None:
        super().__init__(source_name, _SOURCE_TYPE)
        self.paths = paths or ["/remote-web3-jobs"]
    ...
    def _fetch(self, path: str) -> str:
        ...
        try:
            ...
            raise ConnectorSchemaError(f"remote3 fetch failed {url}: {e}") from e
    ...
    def _parse(self, html: str, page_url: str) -> list[dict]:
        ...
        for h in re.findall(r"<h[2-4][^>]*>(.*?)</h[2-4]>", html, re.DOTALL):
            ...
            relevant = any(k in title.lower() for k in [
