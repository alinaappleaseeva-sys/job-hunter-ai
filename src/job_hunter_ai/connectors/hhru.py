"""hh.ru connector (Phase 8 stub).

High-volume Russian board. Anti-bot heavy.
MVP: stub only. Real implementation requires careful acquisition strategy.
"""
...
class HhruConnector(Connector):
    def __init__(self, source_name: str = "hh.ru", jobs: list[dict] | None = None) -> None:
    ...
    def fetch(self, *, cursor_value: str | None = None, limit: int | None = None) -> FetchResult:
        ...
        for idx, job in enumerate(self.jobs):
            ...
            url = job.get("url", f"https://hh.ru/vacancy/{external_id}")
...
def load_sample_hhru_jobs() -> list[dict]:
    return [
        {"id": "hh-1", "title": "Python Backend Developer", "company": "Яндекс", "location": "Москва / Remote"},
    ]
